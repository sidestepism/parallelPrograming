...
const long gemm_threshold = 1 << 15;
const long trsm_threshold = 1 << 15;
const long chol_threshold = 1 << 15;
const long transpose_threshold = 1 << 15;
...
void copy_upper(matrix A, matrix B) {
  long i, j;
  const long M = A.n_rows;
  const long N = A.n_cols;
  assert(M == N);
...
void transpose_loop(matrix A, matrix B) {
  long i, j;
  const long M = A.n_rows;
  const long N = A.n_cols;
  assert(N == B.n_rows);
...
void transpose(matrix A, matrix B) {
  ...
  assert(N == B.n_rows);
  assert(M == B.n_cols);
  if (sizeof(real) * M * N < transpose_threshold) {
    transpose_loop(A, B);
  } else if (M >= N) {
    matrix A_[2], B_[2];
    split_h(A, A_);
